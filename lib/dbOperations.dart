...
Future createUser(
  ...
  try {
    firebaseAuth.createUserWithEmailAndPassword(email: email.text, password: password.text).then((result) {
    ...
    Navigator.push(context, MaterialPageRoute(builder: (context) => const ScreeningBreakdown()));
  } catch(e) {
    if (kDebugMode) {
      print(e);
    }
...
Future enterQuestionData(String age, String gender,
    bool genetic) async {
  final docUser = FirebaseFirestore.instance.collection('UserData').doc(RegisteredHomePage.user.user?.uid);
  final json = {
    'age': age,
    'gender': gender,
    'genetic': genetic,
...
Future enterResults( String condition, String date, String time, String probability) async {
  ...
  final doc = FirebaseFirestore.instance.collection('results')
      .doc(RegisteredHomePage.user.user?.uid).collection('case').doc();

  String urlTime = time.replaceAll(':', '%3A');
  String url = 'https://firebasestorage.googleapis.com/v0/b/skin-safety-scanner/o/'
      '${RegisteredHomePage.user.user?.uid}%2F$date$urlTime?alt=media';
  final json =
  { "Condition": condition,
    "Date" : date,
